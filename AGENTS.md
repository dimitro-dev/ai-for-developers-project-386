...
7. Во время реализации обновляй состояния пунктов в `plan.md`, а выполненное и проверки фиксируй в `result.md`. Каждый завершённый пункт `plan.md` — отдельный коммит (`тип: task-<id> P<NN> — суть`), без смешивания пунктов в одном коммите.
