...
_Черновик, ведётся во время реализации._

Owner-флоу реализуется в `apps/client` по UISpec-киту: фаза 1 (фундамент) и фаза 2 (компоненты)
завершены, идёт фаза 3 (навигация, модель, экраны). Backend, контракт и generated-пакеты не
менялись (API impact `NONE`).
...
10. **Дефолты, не заданные спеками.** Шаг слота в онбординге — 30 минут (спека 03 значения не
